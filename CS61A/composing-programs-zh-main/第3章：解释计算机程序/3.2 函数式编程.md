...
Scheme 支持与 Python 相同的词法作用域规则，允许进行局部定义。下面，我们使用嵌套定义和递归定义了一个用于计算平方根的迭代过程(运用了[什么是古巴算法@20251218_150517](什么是古巴算法@20251218_150517.md)]])：
...
; 定义主窗口和画布
(define frame (new frame% [label "Racket Turtle"] [width 800] [height 600]))
(define canvas (new canvas% [parent frame]))
(define dc (send canvas get-dc))

; 海龟状态变量
(define turtle-x 400)   ; 初始位置：画布中心
(define turtle-y 300)
(define turtle-angle 0) ; 角度：0度指向右方（东）
(define pen-down? #t)   ; 初始画笔落下
...
; 移动海龟并画线（如果画笔落下）
(define (forward dist)
  (let* ([rad (degrees->radians turtle-angle)]
         [new-x (+ turtle-x (* dist (cos rad)))]
         [new-y (+ turtle-y (* dist (sin rad)))])
    (when pen-down?
      (send dc draw-line turtle-x turtle-y new-x new-y))
    (set! turtle-x new-x)
    (set! turtle-y new-y)))

; 旋转海龟（角度增加为顺时针）
(define (right deg)
  (set! turtle-angle (modulo (+ turtle-angle deg) 360)))

(define (left deg)
  (set! turtle-angle (modulo (- turtle-angle deg) 360)))

; 画笔控制
(define (penup) (set! pen-down? #f))
(define (pendown) (set! pen-down? #t))
...
(define (draw-square)
  ...
  (repeat 4 (lambda () (forward 100) (right 90))))
...
(define (draw-star)
  ...
  (repeat 5
          (lambda ()
            (forward 100)
            (repeat 5
                    (lambda () (forward 20) (right 144)))
            (right 144))))

; 显示窗口并绘制示例
(send frame show #t)
(draw-square)   ; 绘制蓝色正方形
(draw-star)    ; 取消注释绘制红色星形
