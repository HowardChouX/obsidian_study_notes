...
- 复制行为：  
    使用 `System.arraycopy` 时，会直接复制 **值本身**，新旧数组完全独立。
    ...
    dest[0] = -1; // 仅修改 dest[0]，src 不受影响
    ```
    

---

#### 2. 对象数组（如 `Integer[]` 或 `String[]`）的存储方式

- 内存结构：数组中存储的是 对象的引用（指针）。
...
- 复制行为：  
    使用 `System.arraycopy` 时，仅复制 引用值，新旧数组共享实际对象。
    
    ```java
    Integer[] src = {new Integer(1), new Integer(2)};
    Integer[] dest = new Integer[2];
    System.arraycopy(src, 0, dest, 0, 2);
    
    dest[0].setValue(-1); // 修改会影响 src[0] 指向的对象
    ```
...
- 本质：多维数组实际上是 数组的数组，外层数组存储的是子数组的引用。
...
- **复制行为**：  
    使用 `System.arraycopy` 复制外层数组时，仅复制子数组的引用。
    ...
    dest[0][0] = -1; // 修改会影响 src[0][0]
    ```


> 关键理解：Java数组的浅拷贝拷贝特性会导致多维数组的子数组共享内存空间，在对象数组中System.arraycopy仅执行浅层拷贝，复制引用，而基本类型数组，复制值。